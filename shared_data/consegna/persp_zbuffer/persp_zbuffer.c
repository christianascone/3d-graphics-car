...
void define_object()
{
  ...
  nface = 6;
  // Il primo indice indica QUALE FACCIA
...
void loadpol(int n) {
  ...
  edges = 0;
  for (k = 1; k <= n; k++) {
    // Leggo valori del secondo punto
    x2 = polx[k];
    y2 = poly[k];
    z2 = polz[k];
    // Se la y coincide, metto uguali x e z e procedo
    if (y1 == y2) {
      x1 = x2;
      z1 = z2;
    } else {
      // Aumento il numero di edges
      edges = edges + 1;
...
void draw_texture(SDL_Renderer *ren, int kf, int i1, int i2, int i3, int i4) {
  ...
  //  2________________1
  // 
  // 0 e 2 sono vertici condivisi, per quello compaiono in entrambi i triangoli
  // nel ciclo
  // Il numero indicato nel disegno viene usato per recuperare da tx, ma gli oggetti
  // i1, i2, i3, i4 sono con un numero incrementato, ma sono gli stessi
  for (k = 0; k < 2; k++)
  { 
    // Primo triangolo
    if (k == 0)
